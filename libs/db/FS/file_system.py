...
class FileSystem(object):
    ...
    def create_asset_workspace(self, sid):
        ...
        for key, value in fs_conf.path_mapping['project'].items():
            if value == sid.get('project'):
                project = key
        workspace_template = self.workspace_init(conf.workspace_template.format(project=project))
        asset_workspace_path = conf.asset_workspace_path.format(project=project, cat=sid.get('cat'), name=sid.get('name'))
    ...
    def create_shot(self, sid, tag='', comment=''):
        ...
        engine_crate = conf.get_soft_by_ext(sid.get('ext'))
        ...
        Template
        """
        path_template = conf.scene_templates.format(project=sid.get('project'), software=engine_crate)
        file_name_template = conf.shot_template_name.format(software=engine_crate, ext=sid.get('ext'))
        ...
        if comment == '':
            comment = "Created on {d} at {h} \n\n".format(d=time.strftime('%A %d/%m/%Y'), h=time.strftime(
                '%H:%M')) + 'Sequence : {sequence}\n' \
                            ...
                            'Task : {task}\n' \
                            'Subtask : {subtask}\n'.format(
                sequence=sid.get('seq'), shot=sid.get('shot'), task=sid.get('task'), subtask=sid.get('subtask'))

        self.create_package_file(sid.get_as('state'), comment, tag)  # TODO PASS A FILE OBJECT

        # CREATE WORKSPACE
        if sid.get('task') == 'comp':
    ...
    def create_asset(self, sid, tag='', comment=''):
        ...
        """
        path_template = conf.scene_templates.format(project=sid.get('project'), software=engine_crate)
        file_name_template = conf.asset_template_name.format(subtask=sid.get('subtask'), ext=sid.get('ext'))
        ...
        if comment == '':
            comment = "Created on {d} at {h} \n\n".format(d=time.strftime('%A %d/%m/%Y'), h=time.strftime(
                '%H:%M')) + 'Category : {cat}\n' \
                            ...
                            'Task : {task}\n' \
                            'Subtask : {subtask}\n'.format(
                cat=sid.get('cat'), name=sid.get('name'), task=sid.get('task'), subtask=sid.get('subtask'))
    ...
    def make_new_version(self, sid, tag, description):
        ...
        if not os.path.exists(new_sid.path):
            ...
            raise pe.PipeException('File already exist !')
        sid.set('version', new_sid.get('version'))
        if tag != '':
    ...
    def create_publish_valid(self, sid_publish):
        ...
        if os.path.exists(path):
            shutil.rmtree(path)
        os.makedirs(path)
        path = Path(path) / (sid_publish.get('version') + '.version')
    ...
    def get_next_version(self, sid):
        ...
        if sid.has_a('version'):
            ...
            sid_result.set('version', max_version)
    ...
    def conform_shot(self, sid):
        ...
        value = "Conformed " + "on {d} at {h} \n\n".format(d=time.strftime('%A %d/%m/%Y'), h=time.strftime(
            '%H:%M')) + 'Sequence : {sequence}\n' \
                        ...
                        'Software : {software}\n' \
                        'Version : {version}\n\n'.format(
            sequence=sid.get('seq'), shot=sid.get('shot'), task=sid.get('task'),
            subtask=sid.get('subtask'), software=engine_conform, version=sid.get('version'))
    ...
    def conform_asset(self, sid):
        ...
        value = "Conformed on {d} at {h} \n\n".format(d=time.strftime('%A %d/%m/%Y'), h=time.strftime(
                '%H:%M')) + 'Category : {cat}\n' \
                            ...
                            'Subtask : {subtask}\n' \
                            'Version : {version}\n\n'.format(
                cat=sid.get('cat'), name=sid.get('name'), task=sid.get('task'), subtask=sid.get('subtask'), version=sid.get('version'))
